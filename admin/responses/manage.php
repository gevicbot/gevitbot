...
if(isset($_GET['id']) && !empty($_GET['id'])){
	$qry = $conn->query("SELECT q.id, r.response_message, q.question, r.urls FROM `questions` q inner join `responses` r on q.response_id = r.id where q.id = {$_GET['id']}");
...
<div class="card card-outline card-info">
	<div class="card-header">
		<h3 class="card-title"><?php echo isset($_GET['id']) ? "Actualizar ": "Crear nueva " ?>consulta a responder</h3>
	</div>
	<div class="card-body">
		<form action="" id="response-form">
			...
			<div class="form-group">
                ...
                <small><i>(Clic en el más "<b>+</b>" para agregar la pregunta)</i></small>
			</div>
			<?php else: ?>
				<div class="form-group">
					<label for="question_field" class="control-label">Pregunta</label>
			...
			<div class="form-group">
				<label for="response_message" class="control-label">Gestión de Respuestas</label>
			...
			<div class="form-group">
				...
				<textarea name="urls" id="urls" cols="30" rows="3" class="form-control" style="resize: none" required><?php echo isset($urls) ? $urls : ''; ?></textarea>
			</div>
		</form>
	</div>
	...
	$(document).ready(function(){
		...
		$('#response-form').submit(function(e){
			...
			if($('input[name="question[]"]').length <= 0){
				...
				$('#question-holder').after("<span class='alert alert-danger err-msg'><i class='fa fa-exclamation-triangle'></i>  Debe agregar al menos una (1) pregunta posible para la respuesta.</span>")
				$('#question').focus();
				return false;
			}
			start_loader();
